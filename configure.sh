...
main() {
    ...
    if [[ "${verify}" == 1 ]]; then
        ...
        # ansible
        envsubst < "${PROJECT_DIR}/tmpl/ansible/kube-vip.yaml" \
            > "${PROJECT_DIR}/provision/ansible/inventory/group_vars/kubernetes/kube-vip.yaml"
...
generate_ansible_hosts() {
    ...
    } > "${PROJECT_DIR}/provision/ansible/inventory/hosts.yaml"
}
